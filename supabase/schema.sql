...
-- and owner sides need a safe, single-call view of their own requests
-- joined with listing/profile info. All three are SECURITY DEFINER,
-- read-only, and loosen no table grants.
-- =========================================================================

create or replace function browse_listings()
returns table (
  id uuid,
  bay_number text,
  street text,
...
set search_path = public
as $$
  select l.id, l.bay_number, l.street, l.area, l.lat, l.lng,
         l.available_start, l.available_end, l.priority_only, l.photo_storage_path,
         l.owner_id, p.display_name, (p.verification_status = 'verified'),
         coalesce(c.cnt, 0)::int
  from listings l
  join profiles p on p.id = l.owner_id
  left join (
    select listing_id, count(*) cnt from swap_requests
    where status in ('pending', 'confirmed') group by listing_id
  ) c on c.listing_id = l.id
  where l.status = 'active'
  order by l.created_at desc;
$$;

revoke execute on function browse_listings() from public, anon;
grant execute on function browse_listings() to authenticated;

create or replace function my_listing_requests()
returns table (
  id uuid,
  listing_id uuid,
  bay_number text,
...
set search_path = public
as $$
  select s.id, s.listing_id, l.bay_number, l.street, s.requester_id, p.display_name,
         s.duration_hours, s.cost_credits, s.is_hot, s.status, s.created_at
...
revoke execute on function my_listing_requests() from public, anon;
grant execute on function my_listing_requests() to authenticated;
